...
class sourceCode():
    ...
    def export(self,):
        ...
        for empid, empname, timestamp, status, date, out_TIME in myAllData:
            emp_ID.append(empid)
            emp_NAME.append(empname)
            emp_TIMEIN.append(timestamp)
            emp_STATUS.append(status)
            emp_DATE.append(date)
            emp_TIMEOUT.append(out_TIME)

        dic = {'ID': emp_ID,'NAME':emp_NAME,'TIMED IN':emp_TIMEIN,'STATUS': emp_STATUS,'DATE':emp_DATE,'OUT':emp_TIMEOUT}
        df = pd.DataFrame(dic)
        df_csv = df.to_csv('E:/MyDTR_EMPID_'+stringPin+'.csv')
